...
describe('resolveAllowedOrigins', () => {
  it('CORS_ALLOWED_ORIGINS를 콤마로 분리하고 trim/빈값 제거한다', () => {
    expect(
      resolveAllowedOrigins({ CORS_ALLOWED_ORIGINS: ' https://a.com , , https://b.com ' } as NodeJS.ProcessEnv),
    ).toEqual(['https://a.com', 'https://b.com']);
  });

  it('CORS_ALLOWED_ORIGINS가 없으면 단일 CORS_ALLOWED_ORIGIN을 사용한다', () => {
    expect(resolveAllowedOrigins({ CORS_ALLOWED_ORIGIN: 'https://only.com' } as NodeJS.ProcessEnv)).toEqual([
      'https://only.com',
    ]);
  });
...
describe('isCorsOriginAllowed', () => {
  ...
  it('명시 목록에 포함되면 환경과 무관하게 허용한다', () => {
    expect(isCorsOriginAllowed('https://app.example.com', ['https://app.example.com'], 'production')).toBe(true);
  });
  ...
  it('명시 목록에 없는 외부 Origin은 거절한다', () => {
    expect(isCorsOriginAllowed('https://evil.com', ['https://app.example.com'], 'production')).toBe(false);
    expect(isCorsOriginAllowed('https://evil.com', ['https://app.example.com'], 'development')).toBe(false);
  });
});
