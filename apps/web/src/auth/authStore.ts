...
import { deleteMyAccount, patchMyProfile } from '../infrastructure/api'

import { readStoredToken, writeStoredToken } from './authSession'

import type { AuthStatus, AuthUser, UpdateProfileInput } from './authContext'

/* =========================================================================
   Auth store (zustand): 토큰(localStorage) + 현재 사용자 + login/register/logout.

   기존 AuthProvider(Context+useState)가 들고 있던 클라이언트 UI 상태를 그대로
   zustand 스토어로 옮긴 것이다. 동작/계약은 동일하다(상태 모양·부수효과·토큰 보관).
   서버 상태(react-query/usePlatformData)는 건드리지 않는다.
...
export type AuthState = {
   ...
   * cancelled 플래그를 둔다(AuthProvider가 cleanup으로 신호를 보낸다). 모듈 레벨
   * 가드로 막으면 두 번째 실행이 건너뛰어 loading에 머문다.
   */
  bootstrap: (isCancelled: () => boolean) => Promise<void>
...
export const useAuthStore = create<AuthState>((set) => ({
  user: null,
  // 저장된 토큰이 있으면 'loading'으로 시작해 부트스트랩(me 복원)을 기다린다.
  status: readStoredToken() ? 'loading' : 'unauthenticated',

  bootstrap: async (isCancelled) => {
  ...
  login: async (email, password) => {
    ...
    set({ user: nextUser, status: 'authenticated' })
    return nextUser
  },

  register: async (email, password, name, organization) => {
    const { token, user: nextUser } = await requestAuth(
      '/auth/register',
