...
// 내부 /inquiry(문의) 페이지의 등록(POST)·목록(GET)을 처리한다. 별도 SDK 의존성 없이
// fetch만 사용하며, 베이스 URL은 VITE_DESK_PLATFORM_URL 로 오버라이드할 수 있다.
// 계약: desk-platform/docs/INQUIRY_INTEGRATION.md (백엔드 LIVE — 기본 베이스 URL로 동작).
...
export const APP_ID = 'family-care'
 ...
 * limit은 1~50(기본 20)으로 클램프한다. 호출부가 AbortSignal로 취소할 수 있다.
 */
