 ...
 * ──────────────────────────────────────────────────────────────────────────
 * npm publish 가 막힌 동안 형제 앱(family-care·offhours·resume·…)에 그대로 복붙해서
 * 쓰는 버전입니다. 워크스페이스 의존(@notifydesk/shared·widget) 0 — 필요한 클라이언트·
 * 상대 시간 포맷·스코프 스타일·아이콘·도메인 타입을 이 파일에 인라인했습니다.
 ...
 *
 * 백엔드 계약(publishable 키 · Authorization: Bearer pk_… + Origin 검사):
 ...
 * prefers-reduced-motion · 대비 ≥4.5:1 · 외부 CSS 프레임워크 0.
 * ──────────────────────────────────────────────────────────────────────────
 */
import {
  useCallback,
...
const WIDGET_VERSION = 'vendor-0.1.0'
...
export function NotificationBell(props: NotificationBellProps): ReactElement {
  ...
  const client = useMemo<NotifyDeskWidgetClient>(
    ...
    [recipientId, publishableKey, endpoint, customFetch]
  )
...
export { NotifyDeskWidgetError }
export type { NotificationDto, InboxDto, UnreadCountDto, MarkReadResultDto }
