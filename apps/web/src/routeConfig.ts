...
export type OperationsRouteModule = 'overview' | 'care' | 'settlement' | 'claims'
export type AdminRouteModule = 'kpi' | 'trends' | 'plans' | 'simulator' | 'summary'
...
export const operationsModuleMeta: Record<OperationsRouteModule, RouteModuleMeta> = {
  overview: {
    title: '전체 현황',
    chip: '현황',
  ...
  settlement: {
    ...
    panelChip: '정산',
    panelTitle: '돌봄비 정산',
    panelDescription: '돌봄 시간과 기준 금액을 입력하면 합계가 바로 계산됩니다.',
...
export const adminModuleMeta: Record<AdminRouteModule, RouteModuleMeta> = {
  ...
  trends: {
    ...
    panelDescription: '정산 합계, 청구 건수, 승인률이 어떻게 달라졌는지 확인합니다.',
  ...
  simulator: {
    ...
    panelDescription: '요금과 상위 요금제 변경 폭을 조정해 예상 월 금액을 확인합니다.',
...
export const routeModeCompositionMeta: Record<'operations' | 'admin', RouteCompositionMeta> = {
  operations: {
  ...
  admin: {
    ...
    compositionDescription: '현황, 월별 변화, 요금, 예상 계산을 쉽게 확인하세요.',
    compositionPanelClass: 'panel-admin panel-ops-composition',
  },
}

export const operationsModuleRoute: Record<OperationsRouteModule, NonHomeRoutePath> = {
  overview: '/operations',
...
export type RouteCompositionSectionProgressState = 'complete' | 'active' | 'upcoming'
...
export const routeHeroMetricLabels: Record<RouteHeroMetricProfile, readonly string[]> = {
...
export type OperationsRoute = {
  path: Exclude<
    NonHomeRoutePath,
    '/admin' | '/admin/overview' | '/admin/trends' | '/admin/plans' | '/admin/simulator'
  >
  section: '운영'
  mode: 'operations'
  stackMode: 'operations'
...
export type AdminRoute = {
  path: Exclude<
    NonHomeRoutePath,
    '/operations' | '/operations/care' | '/operations/settlement' | '/operations/claims'
  >
  section: '관리'
  mode: 'admin'
  stackMode: 'admin'
...
type BaseRouteContext<
  ...
  Modules extends ReadonlyArray<OperationsRouteModule | AdminRouteModule> = ReadonlyArray<
    OperationsRouteModule | AdminRouteModule
  >,
...
export type RouteContext =
  ...
  | BaseRouteContext<
      ...
      OperationsRoutePageBlueprint
    >
  | BaseRouteContext<AdminRoute, 'admin', readonly AdminRouteModule[], AdminRoutePageBlueprint>
...
export const heroTextByMode: HeroTextByMode = {
  home: {
    ...
    description: '가장 먼저 할 일만 보여주고, 바로 다음 동작으로 이어드릴게요.',
  },
  operations: {
    kicker: '돌봄 관리',
    title: '오늘의 돌봄 업무를 빠르게 처리하세요',
    description: '기록-정산-청구 흐름으로 놓치는 단계 없이 진행하세요.',
  },
  admin: {
    kicker: '서비스 관리',
    title: '서비스 운영 상태를 빠르게 확인하고 조치하세요',
    description: '현황·변화·요금을 핵심만 모아 보고 오늘 필요한 조치로 연결하세요.',
  },
}
    ...
    routes: [
      {
        ...
        hero: {
          ...
          description: '오늘 할 일만 보여주고, 놓치기 쉬운 단계를 줄였습니다.',
        },
        emoji: '🗂️',
      ...
      },
      {
        ...
        hero: {
          ...
          description: '날짜, 시간, 기준 금액만 입력하면 가족별 정산 내용을 쉽게 남길 수 있습니다.',
        },
        emoji: '🧮',
      ...
      },
      {
        ...
        hero: {
          ...
          description: '요청, 검토, 승인, 거절 상태를 기록해 놓치지 않게 관리합니다.',
        },
        emoji: '📄',
    ...
    routes: [
      ...
      },
      {
        ...
        hero: {
          ...
          description: '가족 수, 정산액, 승인률처럼 바로 판단 가능한 항목만 보여줍니다.',
        },
        emoji: '📊',
      ...
      },
      {
        ...
        hero: {
          ...
          description: '정산액, 청구 건수, 승인률의 증감 방향을 즉시 파악할 수 있습니다.',
        },
        emoji: '📈',
      ...
      },
      {
        ...
        hero: {
          ...
          description: '월 요금, 할인율, 가구 수를 바꾸면 예상 결과가 바로 반영됩니다.',
        },
        emoji: '💡',
      ...
      },
      {
        ...
        hero: {
          ...
          description: '가격 변화와 상위 요금제 변경 가능성을 조정해 예상 결과를 바로 봅니다.',
        },
        emoji: '🧪',
...
const routeTraversalIndex = new Map(routeTraversalSequence.map((path, index) => [path, index]))
...
export const resolveActiveTopRoutePath = (route: RouteNavItem): TopRoutePath =>
  route.mode === 'admin' ? '/admin' : route.mode === 'operations' ? '/operations' : '/'
...
const homeLandingSections: readonly HomeLandingSectionBlueprint[] = routeNavGroups.map((group) => ({
  section: group.title,
  icon: group.icon,
...
export const getRouteCompositionAtlas = (
  ...
  const activeSectionNavigation = activeSection ? resolveSectionNavigation(activePath) : null
...
export const getRouteCompositionState = (activePath: AppRoute): RouteCompositionState => {
  ...
  const globalRouteProgress = Math.round(((globalRouteCompleted + 1) / globalRouteTotal) * 100)
  ...
  const upcomingInActiveLane = activeLane
    ? activeLane.routes.filter((route) => route.progressState === 'upcoming').length
    : 0
  const sectionSummaries: RouteCompositionSectionSummary[] = orderedAtlas.map((lane, laneIndex) => {
    ...
    const progressRate =
      state === 'active' && activeRouteIndex >= 0
        ? Math.max(0, Math.round(((activeRouteIndex + 1) / lane.routes.length) * 100))
        : state === 'complete'
          ? 100
          : 0
    ...
    return {
      ...
      completedRoutes: lane.routes.filter((route) => route.progressState === 'done').length,
      upcomingRoutes: lane.routes.filter((route) => route.progressState === 'upcoming').length,
      progressRate,
    }
  })
  ...
  if (activeLaneIndex >= 0) {
    ...
    if (previousLane) {
      neighbors.push({
        direction: 'prev',
        path:
          (previousLane.routes.find((route) => route.isEntryPoint) ?? previousLane.routes[0])
            ?.path ?? '/operations',
    ...
    if (nextLane) {
      neighbors.push({
        direction: 'next',
        path:
          (nextLane.routes.find((route) => route.isEntryPoint) ?? nextLane.routes[0])?.path ??
          '/admin',
...
export const getSectionRouteSequence = (path: AppRoute): readonly AppRoute[] | null => {
...
export const getHomeLandingPageBlueprint = (): HomeRoutePageBlueprint => ({
  ...
  copy: {
    ...
    summaryPanelDescription: '가족 돌봄 관리에 필요한 현재 상태를 쉽게 확인합니다.',
    summaryRows: [
      { label: '현재 가구 수' },
      { label: '월 정산 가용성' },
      { label: '청구건수' },
...
export const isHomeRoute = (route: RouteNavItem): route is HomeRoute => route.mode === 'home'
export const isStackModeRoute = (route: RouteNavItem): route is OperationsRoute | AdminRoute =>
  route.mode !== 'home'
...
const getOrderedModulesByMode = (
  ...
  if (mode === 'operations') {
    return operationsModuleSequence.filter((moduleName) => modules.includes(moduleName))
  }

  if (mode === 'admin') {
    return adminModuleSequence.filter((moduleName) => modules.includes(moduleName))
  }
...
export function getOrderedModules(route: OperationsRoute): readonly OperationsRouteModule[]
export function getOrderedModules(route: AdminRoute): readonly AdminRouteModule[]
...
export const getRouteQuickActions = (route: RouteNavItem): readonly RouteQuickAction[] =>
  route.quickActions ?? []
...
export const getRouteSectionProgress = (path: AppRoute): RouteSectionProgress | null => {
  const sectionNavigation = resolveSectionNavigation(path)
  if (!sectionNavigation) {
    return null
  }
...
const resolveSectionNavigation = (path: AppRoute): RouteSectionNavigation | null => {
...
export const getGlobalRouteFlow = (path: AppRoute): RouteGlobalFlow => {
  ...
  return {
    ...
    next: index < routeTraversalSequence.length - 1 ? routeTraversalSequence[index + 1] : null,
    isFirst: index === 0,
...
export function findActiveRouteSection(path: AppRoute): RouteNavGroup | null {
  return routeNavGroups.find((group) => group.routes.some((route) => route.path === path)) ?? null
}
...
export const buildRouteTrail = (route: RouteNavItem): RouteNavItem[] => {
  ...
  const sectionRoot = activeSection ? routeByPath(activeSection.basePath) : null
  return [
    homeRoute,
    ...(sectionRoot && sectionRoot.path !== route.path ? [sectionRoot] : []),
    route,
  ].filter((item, index, list) => index === 0 || item.path !== list[index - 1]?.path)
}
