...
async function requestJson(path, init = {}) {
  ...
  try {
    ...
    throw new Error(
      `Invalid JSON response from ${path}: ${error instanceof Error ? error.message : String(error)}`,
    )
  }
}
...
async function resolveAppUrl() {
  ...
  for (const url of APP_DEFAULT_CANDIDATES) {
    try {
      ...
      if (
        !html.includes('<title>가족 돌봄 운영 플랫폼') &&
        !html.includes('family care platform')
      ) {
  ...
  throw new Error(
    '브라우저 샘플에서 접근 가능한 앱 URL을 찾지 못했습니다. 웹앱이 실행 중인지 확인하세요.',
  )
}
...
async function main() {
  ...
  assert(
    appResponse.includes('<!doctype html>') || appResponse.includes('<html'),
  ...
  for (const settlement of scenario.settlements) {
    ...
    assert(
      created.totalAmount === created.careHours * created.baseRate,
      '정산 총액이 계산값과 일치하지 않습니다.',
    )
  ...
  for (const created of createdCareLogs) {
    assert(
      careLogs.some((item) => item.recipient === created.recipient),
      `돌봄 기록 누락: ${created.recipient}`,
    )
  }
  ...
  for (const created of createdSettlements) {
    assert(
      settlements.some(
        (item) => item.id === created.id && item.totalAmount === created.totalAmount,
      ),
      `정산 누락 또는 수치 불일치: ${created.id}`,
    )
  }
  ...
  for (const created of createdClaims) {
    assert(
      claims.some((item) => item.id === created.id && item.status === created.status),
      `보험청구 누락: ${created.id}`,
    )
  }

  const targetClaim = claims.find((item) =>
    item.recipient.startsWith(`${scenario.claimTargetRecipient} ${marker}`),
  )
  ...
  assert(
    updatedClaim.status === scenario.claimStatusTransition,
    '보험청구 상태 전환 결과가 예상과 다릅니다.',
  )
  ...
  assert(
    overview.totalClaims >= initialOverview.totalClaims + createdClaims.length,
    '청구 건수가 기대값 대비 적지 않습니다.',
  )
  assert(overview.approvedClaims >= 1, '승인 청구가 반영되지 않았습니다.')
  assert(
    overview.monthlyTrend && Array.isArray(overview.monthlyTrend),
    '월별 추세 데이터가 없습니다.',
  )
