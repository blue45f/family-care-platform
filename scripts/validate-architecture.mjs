...
// Architecture/convention guard for family-care, mirroring the sibling repos'
// validate:architecture step. Runs first in `verify` so structural drift fails
// fast (before format/lint/typecheck/test/build).
...
// Required files (docs + tooling single-sources + commit hooks).
const requiredPaths = [
  'README.md',
